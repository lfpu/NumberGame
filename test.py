...
def generate_matrix():
    ...
    while True:
        ...
        # 随机选择起始位置
        start_row, start_col = random.randint(0, 8), random.randint(0, 8)
        matrix[start_row][start_col] = 1
        current_pos = (start_row, start_col)
        current_num = 2

        for _ in range(80):  # 剩余63个数字需要填入
            # 寻找所有可能的下一步位置及其自由度
            neighbors = []
            for d in directions:
                ...
                # 检查是否在矩阵范围内且未被访问过
                if 0 <= next_row < 9 and 0 <= next_col < 9 and matrix[next_row][next_col] == 0:
                    # 计算该位置周围未访问的格子数（自由度）
                    freedom = 0
                    for d2 in directions:
                        rr = next_row + d2[0]
                        cc = next_col + d2[1]
                        if 0 <= rr < 9 and 0 <= cc < 9 and matrix[rr][cc] == 0:
                            freedom += 1
...
for row in matrix:
    print(row)
elements_with_coords = [(x, y, matrix[y][x]) for y in range(len(matrix)) for x in range(len(matrix[y]))]

for x, y, value in elements_with_coords:
    print(f"Element at ({y}, {x}) is {value}")
